...
**Deferred (documented):** automated Paystack **bank transfers** (payouts are admin-approved/manual, per D5); a first-class **Disputes** collection (admins can already refund via `releaseBookingEscrow`); an `endDate`-passed completion **cron** (today the tutor marks complete, or the last session's payout completes it). **Residual concurrency:** the wallet layer is still READ COMMITTED with read-modify-write updates, so under high concurrency reservations can still be lost — but the value-safety clamps/backstops above ensure no path can now **create or over-release money**; true serialization still wants row-level locking / atomic conditional `UPDATE`s across the whole wallet layer. Automated payouts + disputes are Phase 6 "later" per the PRD.
